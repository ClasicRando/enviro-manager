...
data class DbBuild(val entries: MutableList<DbBuildEntry>) {
    suspend fun run(directory: Path, builder: DatabaseBuilder) {
        for (entry in orderedEntries()) {
            builder.runDbBuildEntry(directory = directory, dbBuildEntry = entry)
        }
    }

    private fun orderedEntries(): List<DbBuildEntry> = buildList {
        ...
        while (true) {
            val schemaEntry = entries.withIndex()
